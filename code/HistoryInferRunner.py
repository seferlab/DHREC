...
def runHistory(vararr):
    ...
    #if inter == "bound":
    #   assert scoretimes == None and obstimes != [None]
    #elif inter == "None":
    #   for item in obstimes:
    #       assert item == None
    #   scoretimes = [int(node) for node in scoretimes.split(" ")]
    ...
    stime = time.time()    
    if dist == "dis" and inter == "bound":
       if smodel in ["si","sir","seir","sis"]: 
          if algo == "second-logconcave-external":
             QsapLogconcaveExternal.runner(algopar,curstates,obstimes,G,smodel,infermode,resultfile,runfolder,True)
          elif algo == "second-arbitrary-external":
             QsapArbitraryExternal.runner(algopar,curstates,obstimes,G,smodel,infermode,resultfile,runfolder,True)   
          elif algo == "second-internal":
             pass    
          elif algo == "Independent":
             Independent.runner(inter,G,smodel,curstates,obstimes,resultfile,True)
          elif algo == "MatroidSub":    
             DiscreteBound.runner(algo,algopar,infermode,curstates,G,smodel,resultfile,runfolder,obstimes,True)
       elif smodel in ["miso","mimo"]:
          pass       
    elif dist == "dis" and inter == "None":
       if algo in ["Pcvc","Pcdsvc"]:
    ...
    elif dist == "cont" and inter == "bound":
